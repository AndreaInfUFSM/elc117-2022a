...
Esta prática vai ser entregue em um repositório específico no GitHub Classroom. Clique [aqui](https://classroom.github.com/a/BnWuCJht) para criá-lo. 

Dentro desse repositório, você vai organizar seus arquivos em pastas, conforme as orientações em cada parte da prática. Serão 3 pastas no total: `point`, `association` e `aggregation`. Siga sempre os :warning: padrões de nomes de pastas e arquivos, pois do contrário sua prática não será contabilizada na avaliação.
...
10. Agora restaure para `private` a visibilidade do atributo `r` no programa Circle.java e :no_entry:  **prometa nunca mais usar public em um atributo sem ter um bom motivo para isso!**. 
